...
** 连接器 <<il-connectors>>
连接器是一个系统，该系统根据互联账本数据包目的地地址提供数据包转发服务。数据包跨越链接（账户）在发送方，连接器和接收方被转发。
...
采用预共享密钥协议，发送方和接收方使用一个共享密文来产生支付条件、ILP 报文鉴定、和加密应用数据。使用 PSK，发送者保证满足转移条件是证实接收者获得支付的标志，前提是没有发送者和接收者以外的一方具有共享密文并且发送者不提交 =fulfillment= 。
